    ...
    <div
      ...
      >
        <div ref="bookBox" class="book-box" @click="bookClickHandler(index)">
          <img ref="image" :value="book.selfLink" class="book-image" :src="imageTest(book)" />
        </div>
...
export default {
setup() {
    const { state, postBook, searchByTitle,  } = Search();
    return { state, postBook, searchByTitle,  };
  },
  data() {
    return {
      isBookOpen: false,
  ...
  methods: {
    ...
    //   try {
    //     let linksList = []
    //     for (let index = 0; index < this.state.lengthOfBooks; index++) {
    //       const element = this.$refs.image[index].attributes[2].value;
    //       linksList.push(element)
    //     }
    //     if (this.state.lengthOfBooks > 0) {
    //       let newImageList = await this.newLinkGetter(linksList)
    //       for (let index = 0; index < this.state.lengthOfBooks.length; index++) {
    //         const element = newImageList[index];
    //         console.log(element)
    //         this.$refs.image[index].src = element
