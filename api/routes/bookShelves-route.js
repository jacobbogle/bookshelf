...
router.post("", async (req, res) => {
  ...
  try {
    ...
    if (!testBookInDatabase) {
      ...
      if (!book.user_id.includes(req.cookies.id)) {
        await bookModel.findOneAndUpdate(
          { _id: testBookInDatabase._id.toString() },
          {
            $push: { user_id: req.cookies.id },
          },
          { new: true }
        );
      }
  ...
  try {
    let bookshelfObject = await BookShelf.findOne({ user_id: req.cookies.id });
    if (!bookshelfObject) {
...
router.delete("/:book_id", async (req, res) => {
  ...
  let bookshelf;
  try {
    bookshelf = await BookShelf.findOne({ user_id: req.cookies.id });
  } catch (err) {
